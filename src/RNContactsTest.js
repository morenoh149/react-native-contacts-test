...
export default class RNContactsTest extends Component {
  ...
  addContact () {
    let newContact = {
      ...
      emailAddresses: [{email: 'fromRNContacts1@example.com', type: 'work'}, {email: 'fromRNContacts2@example.com', type: 'personal'}]
    }
    Contacts.addContact(newContact, (err, data) => {
      Contacts.getAll((err, records) => {
        ...
        _.each(newContact, (value, key) => {
          if (Array.isArray(newContact[key])) invariant(contact[key].length === newContact[key].length, 'contact values !isEqual for ' + key)
          else invariant(_.isEqual(contact[key], newContact[key]), 'contact values !isEqual for ' + key)
        })
      })
    })
  }
  ...
  deleteContact () {
    Contacts.getAll((err, contacts) => {
      let contactToDelete = _.find(contacts, (contact) => contact.givenName && contact.givenName.indexOf(PREFIX) === 0)
      if (!contactToDelete) contactToDelete = contacts[0]
      ...
      Contacts.deleteContact(contactToDelete, (err, data) => {
        Contacts.getAll((err, newContactList) => {
          ...
          invariant(newContactList.length === contacts.length -1, 'getAll should return one less result')
          invariant(!_.find(newContactList, {recordID: contactToDelete.recordID}), 'contact should not longer exist')
        })
      })
    })
  }
